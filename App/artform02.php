...
if (comprobarCaracteres($nombre, 3, 30)) {
    enviarError("El nombre debe tener entre 3 y 30 caracteres.", "nombre");
}

if (comprobarVacio($telefono)) {
    enviarError("El teléfono es obligatorio.", "telefono");
}
...
if (comprobarVacio($email)) {
    enviarError("El correo electrónico es obligatorio.", "email");
}
if (!comprobarEmail($email)) {
    enviarError("El correo electrónico no tiene un formato válido.", "email");
}
/*
...
if (comprobarCaracteres($mensaje, 10, 200)) {
    enviarError("El mensaje debe tener entre 10 y 200 caracteres.", "mensaje");
}

if (!$aceptar) {
    enviarError("Debes aceptar los términos y condiciones.", "condiciones");
}

if ($respUser !== $respSystem || empty($respUser)) {
    enviarError("La respuesta a la pregunta de seguridad es incorrecta.", "captcha");
}
...
// 5.- Redirigir a una página de gracias al acabar el proceso asincrono
